...
public class ChasersSchedule {
    public static int calculateDistance(char[] sequence, int speed) {

        int distance = 0;

        for (char unit : sequence) {
            if (unit == 'R') {
                distance += speed;
            } else {
                distance += 2 * speed;
    ...
    public static int solution(int speed, int time) {
        ...
        char[] sequence = new char[time];
        Arrays.fill(sequence, 'R');
        int maxDist = 0;

        for (int i=1; i<time+1; i+=2) {
            sequence[time-i] = 'S';
            maxDist = Math.max(maxDist, calculateDistance(sequence, speed));
        }
        return maxDist;
        }
}
